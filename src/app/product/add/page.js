...
function page() {
    ...
    return <div className="w-[100%] flex items-center justify-center">
        <div className="bg-slate-700 border-none outline-none rounded-xl w-[90%] md:max-w-lg h-fit p-4">
            ...
            <input id="title" type="text" placeholder="Product title" className=" text-white font-semibold input input-bordered input-warning w-full" onChange={(e) => setTitle(e.target.value)} value={title} />
            <label htmlFor="title" className="font-semibold text-slate-100 mt-4 mb-2 block">Description</label>
            <input id="title" type="text" placeholder="Description" className="input input-bordered input-warning w-full text-white font-semibold" onChange={(e) => setDescription(e.target.value)} value={description} />
            <input type="file" className="file-input file-input-bordered file-input-warning w-full mt-6 text-white font-semibold" id="file-input" onChange={(e) => setSelectedFile(e.target.files[0])} />
